...
function setup() {
    ...
    // listen for player point updates
    socket.on('player_point_update', function(message) {

        if (message.id && allPlayers[message.id]) {
            console.log("Got a point update!");
            console.log(message);
            allPlayers[message.id].points = message.points;
        }
    });

    // listen for our point updates
    socket.on('you_got_a_point', function(message) {
        console.log("I got a point update!");
        console.log(message);
...
function draw() {
    ...
    for (let id in allTreasures) {
        ...
        // are we close to a treasure?
        if (dist(x,y,allTreasures[id].x, allTreasures[id].y) < 10) {
            console.log("collecting treasure!", id);
            socket.emit('treasure_collected', {
                id: id
            });
        }
    }
    ...
    for (let id in allPlayers) {
        ...
        fill(0);
        textAlign(CENTER);
        text(allPlayers[id].points, allPlayers[id].x, allPlayers[id].y+25);
    }
