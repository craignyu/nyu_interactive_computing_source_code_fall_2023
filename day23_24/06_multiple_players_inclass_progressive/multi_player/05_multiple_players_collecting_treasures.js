...
const fs = require('fs');
const htmlFile = fs.readFileSync('./public/05_multiple_players_collecting_treasures.html', 'utf-8');
...
io.on('connection', function(socket) {
    ...
    socket.on('treasure_collected', function(message) {
        // make sure the treasure is still available
        if (message.id && allTreasures[message.id]) {
            console.log("Collecting treasure:", message.id);

            // delete the treasure
            delete allTreasures[message.id];

            // create a new treasure
            let treasure = createTreasure();
            allTreasures[treasure.id] = treasure;

            // tell all players about all treasures in the game
            io.emit('all_treasures', allTreasures);

            // give this player a point!
            allPlayers[myId].points += 1;

            // tell all players (except this one) that this player just got a point
            socket.broadcast.emit('player_point_update', {
                id: myId,
                points: allPlayers[myId].points
            });

            // tell this specific player that they got a point
            socket.emit('you_got_a_point', {
                points: allPlayers[myId].points
            });
        }
    })
